...
Every skill that ships in `dist/skills/`: **207 skills**, with its upstream source, licence, and the external services it needs.
...
56 of 207 skills reference something outside the repo. A skill with `none` runs on Claude alone — no keys, no network services.
