...
Every skill that ships in `dist/skills/`: **249 skills**, with its upstream source, licence, and the external services it needs.
...
| `core` | [frankdays/bigslick (first-party)](https://github.com/frankdays/bigslick (first-party)) | MIT | — | 4 | `—` |

Licence texts ship in `licenses/` and in each `upstream/<name>/LICENSE`.

## External dependencies

63 of 249 skills reference something outside the repo. A skill with `none` runs on Claude alone — no keys, no network services.
...
| `client-context` | `bigslick` | frankdays/bigslick (first-party) | MIT | none |
| `client-onboarding` | `bigslick` | frankdays/bigslick (first-party) | MIT | none |
