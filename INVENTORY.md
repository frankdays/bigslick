...
Every skill that ships in `dist/skills/`: **248 skills**, with its upstream source, licence, and the external services it needs.
...
| `core` | [frankdays/bigslick (first-party)](https://github.com/frankdays/bigslick (first-party)) | MIT | — | 3 | `—` |

Licence texts ship in `licenses/` and in each `upstream/<name>/LICENSE`.

## External dependencies

63 of 248 skills reference something outside the repo. A skill with `none` runs on Claude alone — no keys, no network services.
